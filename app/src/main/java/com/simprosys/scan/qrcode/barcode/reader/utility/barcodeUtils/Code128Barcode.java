 ...
 * Code 128 encoding
 */
public class Code128Barcode extends BarcodeCommon implements IBarcode {
    ...
    public Code128Barcode(String input, Context context) {
        setRawData(input);
        getErrors();
        this.context = context;
    }

    public Code128Barcode(String input, TYPES type) {
        this.type = type;
        setRawData(input);
    }
    ...
    private List<Entry> findStartOrCodeCharacter(String s) {
        ...
        if (s.length() > 1 && (Character.isDigit(s.toCharArray()[0])
                || String.valueOf(s.toCharArray()[0]).equals(getChar(200)) && (Character.isDigit(s.toCharArray()[1])
                || String.valueOf(s.toCharArray()[1]).equals(getChar(200))))) {
            ...
            for (Entry row : this.C128_Code) {
                try {
                    ...
                    error(context.getResources().getString(R.string.msgCode128Barcode1) + ex.getMessage());
                }
            }

            if (rows.isEmpty()) {
                error(context.getResources().getString(R.string.msgCode128Barcode2) + s);
            }
        }
        return rows;
    }
    ...
    private String CalculateCheckDigit() {
        ...
        for (int i = 0; i < formattedData.size(); i++) {
            ...
            if (row == null) {
                error(context.getResources().getString(R.string.msgCode128Barcode3));
    ...
    private void breakUpDataForEncoding() {
        ...
        if (this.type == TYPES.C) {
            if (!checkNumericOnly(getRawData())) {
                error(context.getResources().getString(R.string.msgCode128Barcode7));
            }
        ...
        if (this.type != TYPES.DYNAMIC) {
            switch (this.type) {
                ...
                case C:
                    ...
                    break;
                default:
                    error(context.getResources().getString(R.string.msgCode128Barcode4));
                    break;
            }
        } else {
            try {
                ...
                for (int i = 0; i < formattedData.size(); i++) {
                    ...
                    for (Entry row : tempStartChars) {
                        if (row.getA().endsWith(currentCodeString)
                                || row.getB().endsWith(currentCodeString)
                                || row.getC().endsWith(currentCodeString)) {
                            sameCodeSet = true;
                            break;
                        }
                    }
                ...
                error(context.getResources().getString(R.string.msgCode128Barcode5) + ex.getMessage());
            }
        }
    }
    ...
    private String getEncoding() {
        ...
        for (String s : formattedData) {
            ...
            if (E_Row == null) {
                error(context.getResources().getString(R.string.msgCode128Barcode6) + this.type);
                return null;
            }

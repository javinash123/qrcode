...
public class CameraPreview extends SurfaceView implements
    ...
    public CameraPreview(Context context, AttributeSet attrs,
                         CameraWrapper cameraWrapper, Camera.PreviewCallback previewCallback) {
        super(context, attrs);
        init(cameraWrapper, previewCallback);
    }
    ...
    public void setCamera(CameraWrapper cameraWrapper,
                          Camera.PreviewCallback previewCallback) {
    ...
    public void setupCameraParameters() {
        ...
        mPrefs = getContext().getSharedPreferences("SettingPref",
                getContext().MODE_PRIVATE);
        if (mPrefs.getBoolean("InvertScan", false))
    ...
    public int getDisplayOrientation() {
        if (mCameraWrapper == null) {
            // If we don't have a camera set there is no orientation so return
            // dummy value
            return 0;
        }
    ...
    private Camera.Size getOptimalPreviewSize() {
        ...
        List<Camera.Size> sizes = mCameraWrapper.mCamera.getParameters()
    ...
    private Runnable doAutoFocus = new Runnable() {
        public void run() {
            if (mCameraWrapper != null && mPreviewing && mAutoFocus
                    && mSurfaceCreated) {
                safeAutoFocus();
            }
        }
